...
class LightService(BaseService):
    ...
    async def set_led_power(self, enabled):
        if enabled:
    ...
    async def set_led_bluetooth(self, enabled):
        if enabled:
    ...
    async def led_bluetooth_blink(self, blink_rate_ms=500):
        self.led_bluetooth_blinking = True

        while self.led_bluetooth_blinking:
            await asyncio.sleep_ms(blink_rate_ms)
            if self.led_bluetooth_pin.value() == 1:
                self.set_led_bluetooth(False)
            else:
